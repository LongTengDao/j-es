...
function RegularExpressionLiteral(regExp) {
    ...
    return U_FLAG.test(literal)
        ? literal
        : literal.replace(Cf, dynamicallyEscape);
}
...
function ObjectLiteral(object, ValueLiteral, options) {
    if (options) {
        var ES = options.ES || 0;
...
function ArrayLiteral(array, ItemLiteral, options) {
    if (options) {
        var open_close = options.open_close;
        var open_first = options.open_first;
        var comma_next = options.comma_next;
...
function exportify(object, IdentifierValueLiteral, PropertyValueLiteral, options) {
    if (options) {
        var gteES5 = options.ES >= 5;
        var gteES6 = gteES5 && options.ES >= 6;
        ...
        if (hasOwnProperty.call(object, key)) {
            if (isIdentifier(key, false)) {
                ...
                if (gteES6) {
                    ...
                    pairs.push(key + ':' + colon_value + key);
                }
            }
            else {
                pairs.push(PropertyName(key, gteES5) + ':' + colon_value + PropertyValueLiteral(object[key]));
            }
        }
    }
    return named +
